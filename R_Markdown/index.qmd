...
[![](Image/github_Python.png){fig-align="right" width="200"}](https://github.com/zia207/Survival_Analysis_Python/tree/main/Notebook/02_07_00_survival_analysis_introduction_python.ipynb)
:::
...
### Major Types of Survival Analysis

Survival analysis is a statistical branch focused on time-to-event data, handling issues like censoring (incomplete observations). While it encompasses a range of techniques, the major types are broadly categorized into **non-parametric**, **semi-parametric**, and **parametric** methods. These differ in their assumptions about the underlying hazard function (the instantaneous risk of the event occurring). Below, I outline the primary types, their key characteristics, and common applications, based on established statistical practices.

|

Additionally, advanced extensions include:
- **Tree-Based Models**: Such as survival trees or random survival forests, which handle non-linear interactions and are useful for high-dimensional data.
- **Competing Risks Models**: Accounts for multiple possible events (e.g., death from different causes), using methods like the Fine-Gray model.
- **Frailty Models**: Incorporates unobserved heterogeneity (e.g., random effects for clustered data like family studies).

Choosing a type depends on data characteristics (e.g., sample size, censoring rate) and goals (e.g., estimation vs. comparison). For instance, start with non-parametric for exploration, then move to semi-parametric for covariate adjustment. These methods are implemented in software like R (e.g., `survival` package) or Python (e.g., `lifelines` library).

## References

1.  "Survival Analysis: Techniques for Censored and Truncated Data" by John P. Klein and Melvin L. Moeschberger: This book provides comprehensive coverage of both parametric and non-parametric methods in survival analysis.

2.  "Applied Survival Analysis: Regression Modeling of Time-to-Event Data" by David W. Hosmer, Stanley Lemeshow, and Susanne May: This is another authoritative source on the topic, especially on the Cox model and its extensions.

3.  [Survival data analysis](https://www.karlin.mff.cuni.cz/~pesta/NMFM404/survival.html)

4.  [Survival Analysis in R](https://www.emilyzabor.com/tutorials/survival_analysis_in_r_tutorial.html)
